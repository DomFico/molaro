...
// STRIDED SAMPLE of a longer one, and the producer says so in
// `Header.provenance` — as does covalent-bond inference, which changes the edge
// count the status line prints. Both readers live in `webview/provenance.ts`:
// they are the same job (turn one producer-written prose line into a suffix for
// the number it qualifies), and keeping them apart is how the second one gets
// written without the first's hard-won robustness rules. Re-exported here so
// every existing importer of `frameSamplingNote` keeps working.
export { bondInferenceNote, frameSamplingNote } from "./provenance.ts";
export type { FrameSamplingNote, ProvenanceNote } from "./provenance.ts";
